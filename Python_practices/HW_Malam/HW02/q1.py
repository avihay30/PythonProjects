...
# to each player 10 numbers(in range of 1 - 100),
# and printing the winner(the first player that makes the longest ascending arithmetic sequence).
...
def generate_list(ticket_size):
    """ Function generate_list gets a ticket_size,
        and returns a list with length of ticket_size with randomized integer numbers between [1, 99].
    """
...
def play(number_of_players):
    """ Function play is the main function of the program, it gets the "number_of_players"
        from the user and summons the right functions in order to declare the winner
        (the first player that makes the longest ascending arithmetic sequence) of the lottery game.
    """

    # declaring an empty list that will contains all the generated list of each player.
    all_players_list = []
    # declaring an empty list that will contains the sizes of
    # longest ascending arithmetic sequence of each player(according to player index).
    list_of_longest_subarray = []
    # declaring an constant value for readability.
    list_size = 10
    # running in a loop "number_of_players" times.
    for i in range(number_of_players):
        # generating a list for each player an appending it the the "all_players_list".
    ...
    # and that index represent the index of the winner, we added 1 to the index because
    # the count of the players starts with number 1(not 0 like in a list).
    winner_number = list_of_longest_subarray.index(max(list_of_longest_subarray)) + 1
    # printing the winner.
    print("***Player number", winner_number, "is the winner***")


# getting a number of players from the user.
num_of_players = int(input("Please enter the number of players in the lottery game: "))
